...
from intake_forecast.source import ZarrForecastSource, find_previous_cycle_time


HERE = Path(__file__).parent
TEST_ZARR_PATH = HERE / "test_20250401T00.zarr"


def create_test_zarr(path, date=None):
    """Create a test zarr store for testing."""
    if date is None:
        date = datetime(2025, 4, 1, 0)
...
def test_zarr_forecast_source_stepback():
    ...
    assert current_cycle == expected_cycle, f"Expected {expected_cycle}, got {current_cycle}"
...
def test_zarr_forecast_source_stepback_limit():
    ...
    for hour in [6, 12]:
        intermediate_path = HERE / f"test_{datetime(2025, 4, 1, hour).strftime('%Y%m%dT%H')}.zarr"
